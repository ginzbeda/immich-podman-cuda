...
* **Storage tiers:** `immich-server` mounts three host paths — `${HOST_MEDIA_LOCATION}` at `/usr/src/app/upload` (SSD: thumbs, transcodes, profile, backups), `${HOST_LIBRARY_LOCATION}` over `/usr/src/app/upload/library` (HDD: managed originals), and `${HOST_IMPORT_LOCATION}` at `/usr/src/app/external` read-only (HDD: External Library source). Keep `HOST_MEDIA_LOCATION` pointed at a directory Immich owns exclusively — aim it at a folder that already holds photos and Immich will create `thumbs/`, `upload/`, `encoded-video/` and `backups/` interleaved with your own folders.
